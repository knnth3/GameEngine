...
LRESULT Engine::GraphicsWindow::WinProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		m_console->SetDimensions(m_size.Width, m_size.Height);
		m_bResize = true;
		break;
...
void Engine::GraphicsWindow::ToggleConsole()
{
	m_bConsole = !m_bConsole;
}
...
bool Engine::GraphicsWindow::InitGraphics()
{
	...
	m_console = std::make_shared<Console>(10, m_size.Width, m_size.Height);

	universal_struct console(m_console);
	...
	WindowResources::SetConsole(console);
	return true;
}
...
void Engine::GraphicsWindow::RenderConsole()
{
	if (m_bConsole)
	{
		m_graphics->Draw(m_console->GetBackground());
		for (int index = 0; index < m_console->GetSize(); index++)
		{
			m_graphics->Draw(m_console->At(index));
		}
	}
}
