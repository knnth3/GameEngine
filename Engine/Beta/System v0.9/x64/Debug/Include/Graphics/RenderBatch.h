...
namespace Graphics
{
	...
	public:
		DX11Graphics(std::shared_ptr<DX11BufferManager>& bufferManager, std::shared_ptr<DX11ShaderManager> shaderManager, 
			std::shared_ptr<Graphics::DX11DepthStencilState>& depthStencilState);
		~DX11Graphics();
		void Reset();
		bool Initialize(std::shared_ptr<Camera>& camera);
		void AddModel(std::shared_ptr<Model::Model3D>& model);
		void Draw();
	...
	private:
		...
		std::shared_ptr<Graphics::DX11DepthStencilState> m_depthStencilState;
