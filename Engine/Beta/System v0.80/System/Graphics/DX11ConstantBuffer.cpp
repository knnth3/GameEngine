...
Graphics::DX11ConstantBuffer::DX11ConstantBuffer(ID3D11Device* device, ID3D11DeviceContext* context)
{
	m_device = device;
	m_context = context;
	ResetCounter();
...
void Graphics::DX11ConstantBuffer::CreateBuffer(const D3D11_BUFFER_DESC & desc, const std::string uniqueName, D3D11_SUBRESOURCE_DATA * data)
{
	HRESULT result;
