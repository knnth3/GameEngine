...
In the main function, a GraphicsWindow is created. Because GraphicsWindow is pure virtual funtion,
...
private:
	...
	virtual void Render(const std::shared_ptr<Graphics::GraphicsDevice>& graphics)override;
...
```
GraphicsWindow has a few public functions
- The Initialize() function returns true on success
- The Run() function returns when the window closes.
*(The return value currently depends on the postclose function for windows)*

### Required
GrapicsWindow requires a ```std::string``` to be passed through it's initializer to set the window title.
```C++
...
```
Since GrapicsWindow is a pure virtual function, it also requires a few functions to be filled out. These functions are called
by the update thread in GraphicsWindow
```C++
...
*GraphicsDevice should still be able to work if you pass the apps HWND to it. (later on I may add a separate function to allow
a CoreWindow aswell)*
...
Custom Input
------------
Any new class impementations derived of InputDevice can be passed to the Input class AttatchKeyboard() function and will 
...
### Engine supports both 3D and 2D rendering
...
```
### Creating 3D models
3D models can be created/loaded through the MeshLoader static class (reads in .bin files created by the Binary Model Converter in root).
...
### Drawing objects
**Objects can _only_ be drawn in the Render() function!**
...
#### include <Library.h>
- Library is a template class that stores and handles objects creation and destruction using shared pointers.
#### include <universal_map.h>
- Univeral_map is a unordered_map that stores any object wraped in a shared pointer using universal_struct.
